...
public class EasySeg {
    ...
    public static final class Config
    {
         ...
         * 兰州银行自定义热词 词典路径
         */
        public static String[] LZBANK_CustomDictionaryPath = new String[]{"data/dictionary/custom/LZBankCustomDictionary.txt"};;
        /**
         * 2元语法词典路径
         */
        ...
        static
        {
            ...
            try
            {
                ...
                {  // IKVM (v.0.44.0.5) doesn't set context classloader
                    loader = EasySeg.Config.class.getClassLoader();
                }
                ...
                String[] pathArrayLZ = p.getProperty("LZBANK_CustomDictionaryPath", "dictionary/custom/lzbank_hw.txt").split(";");
                String prePathLZ = root;
                for (int i = 0; i < pathArrayLZ.length; ++i)
                {
                    if (pathArrayLZ[i].startsWith(" "))
                    {
                    	pathArrayLZ[i] = prePathLZ + pathArrayLZ[i].trim();
                    }
                    else
                    {
                    	pathArrayLZ[i] = root + pathArrayLZ[i];
                        int lastSplash = pathArrayLZ[i].lastIndexOf('/');
                        if (lastSplash != -1)
                        {
                        	prePathLZ = pathArrayLZ[i].substring(0, lastSplash + 1);
                        }
                    }
                }
                LZBANK_CustomDictionaryPath = pathArrayLZ;
    ...
    {
        return Easy_StandardTokenizer.segment(text.toCharArray());
    }
    ...
    {
        return new Easy_ViterbiSegment();   // Viterbi分词器是目前效率和效果的最佳平衡
    }
